...
function strappress_posted_on() {
	...
	echo '<span class="posted-on"><i class="' . $faType . ' fa-clock"></i>' . $posted_on . '</span>
	<span class="byline"><i class="' . $faType . ' fa-user"></i>' . $byline . '</span>'; // WPCS: XSS OK.
	...
	if ( 'post' === get_post_type() ) {
		...
		if ( $categories_list && strappress_categorized_blog() ) {
			printf( '<span class="cat-links"><i class="' . $faType . ' fa-folder-open"></i>' . esc_html__( '%1$s', 'strappress' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}
	}

	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		echo '<span class="comments-link"><i class="' . $faType . ' fa-comments"></i>';
...
function strappress_entry_footer() {
	...
	if ( 'post' === get_post_type() && is_single() ) {
		...
		if ( $tags_list ) {
			printf( '<span class="tags-links"><i class="' . $faType . ' fa-tags"></i>' . esc_html__( '%1$s', 'strappress' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}
